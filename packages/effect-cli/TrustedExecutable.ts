...
export class TrustedExecutableError extends Schema.TaggedError<TrustedExecutableError>()("TrustedExecutableError", { message: Schema.String }) {}
...
export const trustedExecutable = Effect.fn("TrustedExecutable.trustedExecutable")(function*(name: string, reviewedRepoPath = process.cwd()) {
  ...
  return yield* new TrustedExecutableError({ message: `could not resolve trusted ${name} executable outside the reviewed checkout; use the explicit tool override` })
})
