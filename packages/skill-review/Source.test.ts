...
it("captures exact text, binary bytes, and symlink targets without following supporting links", async () => {
  const result = await runtime.runPromise(Effect.gen(function*() {
    ...
    yield* fileSystem.writeFileString(path.join(skill, "BASE.md"), "\uFEFF# Audited base\r\nDifferent from a variant.  ")
  ...
  expect(result.entry).toBe("\uFEFF# Audited base\r\nDifferent from a variant.  ")
  expect(result.files.find((file) => file.path === "BASE.md")?.content).toBe(result.entry)
  expect(result.files.find((file) => file.path === "variants/base.md")?.content).toBe("\uFEFF# Original\r\nExact bytes\r\n")
