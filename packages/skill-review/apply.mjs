...
export async function prepareApply({ name, revision, directory, root, readDetail = detail }) {
  ...
  return { directory, revision, sourceDrift: source.fingerprint !== record.source.fingerprint, next: "The pinned master is in candidate/BASE.md. Read it and the model guides to produce all five complete variants. Reconcile supporting-file drift and independently exercise the variants, then apply this pinned plan." };
}
