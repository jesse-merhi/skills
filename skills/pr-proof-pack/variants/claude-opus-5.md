...
Make the existing reviewer-visible PR account accurate for its current head,
refreshing only stale or missing proof under the caller's publication authority.
When called from a delivery workflow, return status to that caller without
claiming its final readiness result; on direct use, open with the PR and
freshness classification.

Keep the PR body, screenshots, recordings, diagrams, and text evidence to the
smallest set that proves the changed behavior. Return `current`, `refreshed`,
or `blocked` with affected PRs and remaining review/sign-off needs. Run the
specified freshness, capture, upload, and rendered-readback checks only. Use
named media or diagram skills directly when needed; do not spawn review agents.
