...
describe("PR net diff change breakdown", () => {
  ...
  it("classifies root UI paths without treating app API routes as UI", () => {
    for (const [path, expected] of [
      ...
      ["src/app/app.routes.ts", /actual product pixels/], ["src/app/card/card.component.ts", /actual product pixels/], ["src/app/card/card.directive.ts", /actual product pixels/], ["src/components/logo.svg", /actual product pixels/], ["src/app/templates/application.hbs", /actual product pixels/], ["src/components/my-element.ts", /actual product pixels/], ["src/components/server/client.ts", /Practical backend proof required/], ["app/src/main/java/com/example/MainActivity.java", /actual product pixels/], ["app/MainPage.cs", /actual product pixels/], ["app/src/main/java/com/example/UserService.java", /Practical behavior proof required/], ["src/jobs/components/job-card.ts", /Practical UI proof required/], ["docs/api.md", /Practical documentation proof required/], ["specs/server.mdx", /Practical documentation proof required/], ["docs/queue.md", /Practical documentation proof required/]
    ] as const) assert.match(proofHintForPath(path), expected)
    assert.match(proofHintForPath("components/Label.tsx"), /where appearance is not the claim, use copyable text instead/)
    for (const path of ["src/server.ts", "src/jobs/worker.ts", "docs/guide.md", "src/unknown.ts"]) {
      assert.match(proofHintForPath(path), /actual product pixels.*recording.*both/s)
    }
    assert.match(proofHintForPath("src/screens/ProfileScreen.tsx"), /baseline is meaningful and reproducible.*otherwise state the constraint.*actual entry point and PR outcome/s)
    for (const path of ["src/api.ts", "src/jobs/worker.ts"]) assert.match(proofHintForPath(path), /baseline is meaningful and reproducible.*otherwise state the constraint.*actual entry point and PR outcome/s)
  })
})
