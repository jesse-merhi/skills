...
const proofKind = (path: string) => {
  ...
  if (["/routes/", "/components/", "/app/", "/pages/", "src/styles", ".css"].some((marker) => lower.includes(marker))) return "Practical UI proof required: capture a deliberately paced manual interaction video plus screenshots of every distinct changed state with a browser or device, then upload them as provider-hosted evidence. Tests, builds, CI, and automated E2E output are supporting checks only."
