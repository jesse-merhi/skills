...
const proofKind = (path: string) => {
  ...
  if (["/routes/", "/components/", "/app/", "/pages/", "src/styles", ".css"].some((marker) => lower.includes(marker))) return "Practical UI proof required: capture a tightly edited, natural-speed manual interaction video with a browser or device, then upload it as provider-hosted evidence; capture matched direct-base/PR behavior when comparable, and add still images only for a readable before/after comparison or static state. Tests, builds, CI, and automated E2E output are supporting checks only."
