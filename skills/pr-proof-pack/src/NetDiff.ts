...
const proofKind = (path: string) => {
  ...
  if (["/routes/", "/components/", "/app/", "/pages/", "src/styles", ".css"].some((marker) => lower.includes(marker))) return "Practical UI proof required: upload a deliberately paced manual interaction video plus screenshots of every distinct changed state with the selected interactive browser. Tests, builds, CI, and automated E2E output are supporting checks only."
