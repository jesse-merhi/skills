...
const command = Command.make("pr-net-diff", {
  ...
  markdown: Flag.boolean("markdown"), json: Flag.boolean("json")
}, Effect.fn("prNetDiff.handler")(function*({ paths, base, head, json }) {
  ...
  yield* Console.log(json ? JSON.stringify(report, null, 2) : renderMarkdown(report))
})).pipe(Command.withDescription("Count changed files and added/deleted lines by category from the merge base to the selected head"))
