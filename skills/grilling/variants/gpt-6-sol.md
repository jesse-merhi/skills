...
Use `spawn_agent` with `agent_type: "investigator"` for the blocking factual question. Pass the objective, worktree and revision or source, owned scope, constraints, acceptance criteria and relevant evidence. Keep reusable guidance in the role definition. Continue independent questions while it runs; keep question sequencing and synthesis here and decisions with the user. If the role is unavailable, report it and establish the fact locally.
