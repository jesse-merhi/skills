...
Review the complete changed architecture before concluding. Batch independent
reads and contract checks. During a long review, report only a change in
evidence or direction. Verify unfamiliar framework behavior from current
sources. Recommend concrete targeted changes; refactor only when implementation
is authorized.
