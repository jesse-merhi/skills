...
Identify one or two evidence-backed structural improvements without widening
the requested review scope or refactoring without authority. When used as a
review lens, return candidates to the owning reviewer without separate
user-facing progress or a final verdict. On direct use, lead with the strongest
finding and concrete paths.

Store a concise Obsidian note only when durable project knowledge warrants it;
otherwise return the proposed note body and path as specified below. Current
code, change pressure, and the migration/test strategy are the required
evidence, not grounds for a second generic review. Keep this analysis in the
current session and do not delegate it.
