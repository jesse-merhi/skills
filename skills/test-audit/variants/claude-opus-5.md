...
Decide which tests own real, reachable product risks at the narrowest useful
level. When called as a review lens, return the required buckets to the owning
reviewer without separate user-facing progress or a final verdict. On direct
use, lead with the actionable portfolio changes.

Use the referenced output buckets as the complete report; create no parallel
artifact. Run focused tests and typecheck or lint only after authorized edits
and where the workflow requires them, without a generic suite rerun. Perform
portfolio ownership judgment in the current context and do not delegate it.
