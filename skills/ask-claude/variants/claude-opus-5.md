...
Run only the explicitly requested Claude ACP session, using the read or write
authority defined below. Before launch, give one short line naming the session
purpose. Update the user only if authentication, write scope, or session cleanup
changes the result.

Return Claude's result with the evidence and cleanup status needed to use it;
do not add a second report around the answer. Keep the ACP brief and any saved
artifact to the requested constraints and output shape. Inspect returned edits
and close the temporary session as required below, without a generic recheck.
This workflow requires one external Claude session and forbids substituting or
adding in-chat agents.
