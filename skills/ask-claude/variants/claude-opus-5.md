...
Keep the brief and requested output bounded to the user’s scope. Give Claude a self-contained brief: objective, checkout, relevant files, constraints, expected answer and evidence, and whether writes are authorized. Run from the intended checkout. Use the current Claude configuration without choosing a model:
