...
Drive the harness-native review phase to two consecutive clean runs on the same
tree, fixing only accepted findings. When called by `code-review`, return pass
outcomes, fixes, and blockers to that coordinator without taking over its
user-facing final response. On direct invocation, report only a changed verdict,
fix, or stop reason.

Persist findings and phase state through the required CLI rather than a
separate report. The native clean streak and affected validation are the
workflow's evidence, not redundant rechecking. Use only the selected
harness-native review session; do not add ad hoc subagents or verification
agents.
