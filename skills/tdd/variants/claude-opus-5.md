...
Deliver the requested behavior through one vertical red-green-refactor cycle at
a time and keep every edit inside the authorized seam. Open with the seam,
observable outcome, and first failing test. Update only when red becomes green,
the seam changes, refactoring alters the contract, or a blocker appears.

The final response leads with the working behavior and names the retained tests
and validation. Save only product code and tests that own reachable regressions;
do not create a separate TDD report. Each red, green, refactor, and affected
validation step below is required, but no generic final recheck is. Run the
cycles in the current session without delegation.
