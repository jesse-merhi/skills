...
workflow for that version. A configuration entry alone does not prove Context7
is callable; verify it with an actual tool call. If it is unavailable, use the
official docs or installed package source. Do not install or start a local
Context7 helper as a fallback.
