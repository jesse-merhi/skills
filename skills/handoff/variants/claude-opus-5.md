...
Create or prepare the requested full independent session without turning any
part of the handoff into in-chat delegation. Give one short update before
preparing the handoff, then update only if the target surface, isolation, or
launch status changes.

Keep the temporary handoff document compact and link existing evidence instead
of copying it. The final response must name its path, relationship, surface,
placement, isolation, and verified launch state. Perform only the launch and
state checks required below. Subagents, background agents, and delegated
workers are forbidden; the destination session is the handoff itself.
