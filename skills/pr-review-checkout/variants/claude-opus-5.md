...
Open the requested PR in its real worktree and VS Code active-pull-request view
without reviewing, merging, or commenting on the user's behalf. Open with one
short line naming the PR and worktree. Update only if checkout identity,
worktree creation, or VS Code activation is blocked.

Keep the reviewer orientation brief and save no report artifact. Return the
opened PR, worktree path, view state, and any cleanup command or limitation.
Verify the real branch and active PR view as required below; do not add a generic
review of the change. This UI setup remains in the current session and is not
delegated.
