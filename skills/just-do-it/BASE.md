...
When this skill runs in the main session, assign one configured implementer as execution owner for the whole cohesive change. That owner performs the steps below through integration, shared verification, review-record mechanics, commits and authorized delivery. The main coordinator plans, judges `code-review`, and gives Jesse the material decisions and inspection checkpoint. An agent already assigned the whole workflow is its execution owner and does not create another mandatory owner.

Use the configured investigator only for a useful independent question. Supply every worker the objective, worktree and revision, scope, constraints, acceptance criteria and relevant evidence. Do not launch a fixed agent tree. Route independent review through `code-review`.

If the execution role is unavailable or fails, reassign the remaining execution to a supported owner or report the blocker. The main coordinator may continue read-only planning and review, but does not take over implementation. Do not reconstruct role prompts or change live configuration.
