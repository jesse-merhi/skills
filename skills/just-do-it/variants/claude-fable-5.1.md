...
When this skill runs in the main session, use `Agent` with `subagent_type: "implementer"` to assign one execution owner for the whole cohesive change. That owner performs the steps below through integration, shared verification, review-record mechanics, commits and authorized delivery. The main coordinator plans, judges `code-review`, and gives Jesse the material decisions and inspection checkpoint. An agent already assigned the whole workflow is its execution owner and does not create another mandatory owner.

The main coordinator uses `Agent` with `subagent_type: "investigator"` only for a useful independent question because the execution owner cannot use `Agent`. The execution owner handles ordinary in-scope investigation itself. Pass every worker the objective, worktree and revision or source, owned scope, constraints, acceptance criteria and relevant evidence. Do not launch a fixed agent tree. Route independent review through `code-review`.

If the execution role is unavailable or fails, reassign the remaining execution to a supported owner or report the blocker. The main coordinator may continue read-only planning and review, but does not take over implementation. Do not rebuild role prompts or change live configuration.
...
2. Finish implementation and confirmed repairs, commit locally, and resolve the planned PR base. Use `code-review` to establish applicable evidence for the current head and full PR diff, including all branch commits; do not default to the latest commit’s parent. This invocation authorizes the coordinator to assess reuse and run any needed review. Complete applicable local validation, reusing valid results. For web or native UI, use `frontend-ui-validation` and retain its proof.
...
4. The execution owner prepares the PR link and evidence for a self-contained inspection checkpoint before starting CI. The main coordinator gives that checkpoint to Jesse, explaining the user-visible result, observed problem, important design decisions, local proof, remaining platform gaps and what deserves his attention. Describe the final CI run and any real spending constraint so his go-ahead authorizes that explained run once. Allow him to inspect; silence, an earlier merge reaction or general delivery authority is not this go-ahead. Return feedback to the execution owner, who rechecks affected local evidence and review before CI; obtain a revised go-ahead if feedback materially changes the explained run. Preserve still-valid approval when it does not.
