...
Finish the whole bounded workflow through a genuinely reviewer-ready PR. Batch
independent reads and checks. During long implementation, review, proof, or CI
work, report each meaningful checkpoint, evidence change, or blocker. Verify
current tool and dependency behavior from source. Prefer targeted edits and
keep code, tests, and publication inside the granted authority.
