...
2. Finish implementation and confirmed repairs, commit locally, and complete `code-review` on the current head. This invocation authorizes that review. Run applicable local validation; reuse evidence that still applies. For web or native UI, use `frontend-ui-validation` and retain its practical proof.
