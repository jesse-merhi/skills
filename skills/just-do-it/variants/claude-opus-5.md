...
Carry one explicit, cohesive change from its current checkpoint to a genuinely
reviewer-ready PR within the publication authority below. Open with the
checkpoint being resumed. Report only meaningful transitions in implementation,
review, proof, CI, readiness, or an external blocker.

Keep the PR body and proof pack to the smallest complete reviewer-facing account.
The final response starts with the PR URL and readiness state, then exact head,
behavioral proof, review, CI, gates, and remaining owner action. Run the named
checkpoint validations and exact-head fixed point only; do not add generic
rechecks. All workers required by `code-review` are allowed, but this workflow
adds no discretionary agents of its own.
