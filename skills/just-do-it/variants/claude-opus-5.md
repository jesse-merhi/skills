...
For useful independent work, use `Agent` with `subagent_type: "investigator"` for questions and `Agent` with `subagent_type: "implementer"` for owned changes and focused tests. Pass the objective, worktree and revision or source, owned scope, constraints, acceptance criteria and relevant evidence. Keep sequencing, integration, shared verification and delivery here; route independent review through `code-review`. Do not launch a fixed agent tree. If the role is unavailable, report it and continue suitable work locally. Do not rebuild role prompts or change live configuration.
...
2. Finish implementation and confirmed repairs, commit locally, and resolve the planned PR base. Use `code-review` to establish applicable evidence for the current head against the full planned PR comparison, including all branch commits; before PR creation, do not default to the latest commit’s parent. The coordinator decides what evidence to reuse and what further review is needed. This invocation authorizes that assessment and any review it requires. Run applicable local validation; reuse evidence that still applies. For web or native UI, use `frontend-ui-validation` and retain its practical proof.
