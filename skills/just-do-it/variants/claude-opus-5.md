...
When useful bounded work can run independently, select Codex's configured `investigator` for a code or documentation question and `implementer` for an owned change and its focused tests. Select the named role through the launcher's role field (`agent_type` on `spawn_agent`); its definition owns reusable instructions, skill references, model and effort. Supply the objective, worktree and revision, owned scope, constraints, acceptance criteria and relevant evidence. Keep sequencing, integration, shared verification and delivery with the coordinator; do not launch a fixed agent tree. Route independent review through `code-review`.

In Claude Code, invoke the native `Agent` tool with `subagent_type: "investigator"` or `subagent_type: "implementer"` when the matching optional named definition is available. Put the same bounded assignment in `prompt`; the definition owns reusable role instructions. Do not launch either role just because it is installed. Keep sequencing, integration, shared verification and delivery with the coordinator, and route independent review through `code-review`.

Use only roles exposed by the current launcher. If a configured role is unavailable, report the limitation and continue suitable work locally; do not rebuild its prompt or change live configuration. OpenClaw and other harnesses use their own supported launch settings under `AGENTS.md`; Codex and Claude role selectors are not portable spawn arguments.
