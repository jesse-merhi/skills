...
2. Finish implementation and confirmed repairs, commit locally, and resolve the planned PR base. Use `code-review` to simplify the full PR change, then establish independent standards and requirements/correctness assessments of the resulting code, including all branch commits. Preserve applicable review evidence and recheck only conclusions affected by later changes. Complete applicable local validation, reusing valid results. For web or native UI, use `frontend-ui-validation` and retain its proof.
