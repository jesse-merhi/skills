---
name: just-do-it
description: 'Take one well-defined, cohesive change from implementation through exact-head code review, proof, CI, and a reviewer-ready PR. Use only when the user explicitly invokes this skill.'
---

# Just Do It

Take a small, well-defined change all the way from a reproduced problem to a
non-draft PR that is ready for Jesse to inspect. Keep moving through ordinary
...
implementation when a missing product decision, breaking change, new
...
1. Freeze the change and delivery target.

   Read the applicable instructions, inspect the relevant code and installed
   dependencies, reproduce the problem or record the observable baseline, and
   write down the acceptance behavior. Confirm the worktree is dedicated, the
   feature branch is not the default branch, the intended base is current, and
   one PR is the right delivery shape. Preserve unrelated local changes.

   Done when the before behavior, expected after behavior, scope boundary,
   validation targets, branch, base, and single-PR shape are concrete.
...
2. Implement the smallest complete fix.
   ...
   contract when that is practical. Exercise the changed behavior directly,
   then run the smallest sufficient local validation. Stop on the first test
   failure and diagnose it before continuing.
...
3. Open a reviewable draft PR.
   ...
   normal push to the feature branch and create one draft PR, or update the
   existing Jesse-authored PR for that branch. Give the draft a truthful title
   and enough initial problem/fix context for review. Never push to the default
...
4. Run the full code review automatically.

   Load `code-review` and complete both of its until-clean phases, accepted
   fixes, final validation, exact-head closeout, and authorized final push.
   This explicit `just-do-it` invocation is the user's review decision: do not
   pause to ask whether to run `code-review`, and do not replace it with an ad
   hoc review, CI, a proof pack, a repository bot, or `autoreview`.
...
5. Build the final proof pack.

   Load `pr-proof-pack` against the final direct-base net diff. Refresh the PR
   only when its proof is stale or missing. Show the observed broken and fixed
   behavior for a reproducible bug, use the simplest evidence format that
   preserves the claim, and inspect the rendered result. Then mark the PR ready
   for review.
