...
Read applicable skills and scoped AGENTS.md or CLAUDE.md instructions. Batch independent reads; consult linked documents when they govern the current decision. Resolve contradictions at their owner instead of adding another rule.
...
Finish all authorized variant edits with targeted changes. During long work, report meaningful progress without narrating routine reads. Keep edits within the requested scope. Installation and model switching follow repository `INSTALL.md` and README; a prompt edit does not authorize either.
...
For skills, `BASE.md` is the human-owned, model-neutral source of truth. Read it and the shared resources first, then the applicable provider guidance: [Astra](references/gpt-6-astra.md), [Sol](references/gpt-6-sol.md), [Luna](references/gpt-6-luna.md), [Fable](references/claude-fable-5.1.md), or [Opus](references/claude-opus-5.md).
...
Choose skill execution roles using the applicable `AGENTS.md` model policy. Keep shared instructions available to coordinators and reviewers as well as workers; a model variant adapts wording, not ownership or permission to execute the workflow.

File presence records coverage. Keep root `SKILL.md` linked to `variants/gpt-6-astra.md` for runtime discovery; it is not the base. The materializer selects an already-authored variant, not a generated base or a model call. Validate affected prompts against the base through independent agent exercises and validate installation with the materializer test.
