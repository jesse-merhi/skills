...
- Ask for brief progress updates when a long tool run would otherwise go silent, and ask for independent tool calls to be batched when the harness pays per turn.
- Tell the model to finish the whole authorized task when it tends to describe the next step or ask again for permission already granted.
...
- Bound extra scope, tests, and whole-file rewrites. At low effort, explicitly require search for current or unfamiliar facts.
