...
Run only the explicitly requested Codex ACP session, using the read or write
...
Return Codex's result with the evidence and cleanup status needed to use it;
do not wrap it in a second report. Keep the ACP brief and any saved artifact to
the requested constraints and output shape. Inspect returned edits and close
the temporary session as required below, without a generic recheck. This
workflow requires one external Codex session and forbids substituting or adding
in-chat agents.
