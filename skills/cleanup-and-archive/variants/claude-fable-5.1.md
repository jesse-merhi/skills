...
Use the current session unless the user names another one. A request to create or edit this skill is not a request to run it. Invoking this workflow authorizes cleanup within [cleanup](../cleanup/SKILL.md)'s boundaries and archiving the selected session; carry both stages through to completion without asking again for already authorized actions. Keep progress updates brief if teardown takes time.
