...
Use the current session unless the user names another. Creating or editing this skill does not invoke it. Invocation authorizes [cleanup](../cleanup/SKILL.md) and archiving the selected session; finish both without reapproval. Ask for decisions cleanup reserves for the user. Keep progress updates brief if teardown takes time.
