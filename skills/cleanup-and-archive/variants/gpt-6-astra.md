...
Use the current session unless the user names another one. A request to create or edit this skill is not a request to run it. Invoking this workflow authorizes cleanup within [cleanup](../cleanup/SKILL.md)'s boundaries and archiving the selected session; resolve routine choices from that context and existing authorization without asking again. Preserve cleanup’s decisions that require the user.
