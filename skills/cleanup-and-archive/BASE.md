...
Use the current session unless the user names another one. A request to create or edit this skill is not a request to run it. Invoking this workflow authorizes cleanup within [cleanup](../cleanup/SKILL.md)'s boundaries and archiving the selected session; do not ask again for those already authorized actions.
...
4. Archive through the available native session tool using its exposed schema. If archiving is unavailable or fails, report cleanup's actual result and leave archiving explicitly incomplete. Do not simulate archiving by deleting transcripts or editing internal session databases.
