...
Produce one independent cold review with zero implementation context. This is a
review helper: return its findings and rejected candidates to the caller, and do
not emit separate user-facing progress or claim the owning workflow's final
verdict.

Keep the output in the referenced finding format and record only the changed
files actually assessed; do not save a parallel review narrative. The neutral
review and coverage attestation are the complete proof for this invocation.
Use the one isolated reviewer required below, with the documented self-review
fallback only when unavailable; do not add further agents.
