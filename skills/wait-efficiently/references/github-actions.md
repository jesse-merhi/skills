...
Resolve the PR's current head and required checks. Set an interval suited to workflow duration and urgency. Omit `--required` when all checks matter; use `--fail-fast` with the PR watch only when the first failure answers the task. For several checks, use one PR watch rather than a loop per check.
