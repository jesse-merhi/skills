...
Hold the active wait until completion, an actionable state, or the selected
deadline. If the calling workflow has not already named what is being awaited,
give that one initial status; otherwise add no opening update. Heartbeats and
unchanged progress never justify another message.

Return the terminal or actionable state to the caller rather than claiming the
owning task's final result. This skill creates no saved deliverable. The single
event-driven hold is the proof that waiting continued; do not add polling or a
post-wait recheck. Waiting on a command or agent does not authorize spawning any
additional agents.
