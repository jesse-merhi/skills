...
Implement the authorized plan through disjoint vertical slices while the
orchestrator retains dependency order, integration, and final behavior. Open
with the slice ownership map. Update only when a slice completes, a contract
changes, integration fails, or a blocker changes the dependency graph.

Do not create a separate report artifact; return the integrated behavior,
validation, worker outcomes, and unfinished work. Validate each slice at its
owned seam and the final integrated result as required below, without a generic
review layer. Dispatch only the workers authorized by the user and assigned in
the slice plan; do not add verification-only agents.
