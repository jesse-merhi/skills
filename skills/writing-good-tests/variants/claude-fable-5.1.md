...
Use the configured `test_executor` role at GPT-5.6 Luna, max effort. If the harness exposes explicit model and effort settings instead, launch a fresh unnamed worker with those settings and point it to this skill. Verify the actual launch settings; a task name or prompt cannot override a pinned role. If neither route is available, disclose that limitation and keep suitable execution local without changing live configuration. Claude’s native `Agent` model selector does not select GPT models; do not label a Claude worker as Luna. Use an already-authorized launcher that supports these settings when available.
