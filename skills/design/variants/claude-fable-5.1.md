...
Choose the one design mode that matches the request and load only that mode's
reference. Batch independent context reads when useful. Complete the selected
design task, but do not widen it into another mode or implementation work the
