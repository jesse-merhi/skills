...
Assess the changed code for present reading and maintenance cost, then recommend
only a concrete lower-load replacement that passes the plausibility and proxy
tests. As an internal review lens, return candidates to the owning reviewer
without user-facing progress or a separate final verdict. On direct use, lead
with actionable findings and paths.

Create no saved report; keep evidence beside each recommendation. Existing
types, boundaries, tests, and the referenced decision checks are sufficient
proof, so do not add a generic second review. Perform this lens in the current
