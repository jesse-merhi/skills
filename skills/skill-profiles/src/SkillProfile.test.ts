...
describe("skill catalogue discovery", () => {
  ...
  it("adds the trusted projects listed in the Codex config", async () => {
    ...
    try {
      ...
      assert.deepStrictEqual(discoverCatalogue(options).map((skill) => skill.name), ["trusted-skill"])
    } finally {
      await rm(directory, { recursive: true, force: true })
    }
  })
...
describe("renderers", () => {
  ...
  it("renders the agent file and repeats byte for byte", async () => {
...
enabled = false
`)
    assert.strictEqual(await Effect.runPromise(renderAgentFile(fixedProfile("# Demo\n\nBody line.\n"), ["/a/SKILL.md"])), rendered)
  })
...
describe("skills-profile command", () => {
  it("checks, installs, and refuses to write without consent", async () => {
