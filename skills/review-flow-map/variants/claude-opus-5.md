...
Trace the changed runtime flows, contracts, side effects, and proving checks
before findings are written. This is internal review context: return the map to
the owning reviewer without user-facing progress or a separate final verdict.

Use the referenced map shape as the only deliverable and keep it to
change-relevant entrypoints and risks. Trace each claimed edge to code,
configuration, or a contract; do not add a generic second review. Build the map
in the current context without delegation.
