...
Turn the settled conversation and current project evidence into one
implementable spec without reopening resolved decisions. Open with the proposed
spec scope and evidence sources. Update only when a missing product decision,
test seam, UI validation bar, or delivery shape needs the user.

Keep the saved Obsidian spec to the referenced template and the detail required
for implementation; omit conversational history and duplicate summaries. Return
the final note path, or the Markdown body and proposed path when publishing is
blocked. Check seams, acceptance behavior, and PR shape as part of drafting,
without a separate generic verification pass. Write the spec in the current
session without agents.
