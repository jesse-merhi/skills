...
const reviewStart = Command.make("start", reviewStartFlags, args => withScopeDb(args.db, args.repoPath, Effect.gen(function*() {
  ...
  yield* Console.log(JSON.stringify({ reviewId: review.reviewId, phase: review.phase, head: review.head, status: review.status, resumed: review.resumed, limits: yield* reviewLimits(reviewRun(review), review.phase) }))
})))
const reviewStatus = Command.make("status", reviewHandle, args => withDb(args.db, Effect.gen(function*() {
  yield* initialize()
  const review = yield* getReview(args.review)
  yield* Console.log(JSON.stringify({ reviewId: review.reviewId, phase: review.phase, head: review.head, status: review.status, outcome: review.outcome, evidence: review.evidence, launched: review.launched === 1, ...(review.launched === 1 ? { report: yield* nativeReportPath(args.db, review.reviewId) } : {}), limits: yield* reviewLimits(reviewRun(review), review.phase) }))
})))
const reviewFinish = Command.make("finish", { ...reviewHandle, outcome: Flag.choice("outcome", ReviewOutcome.literals), evidence: Flag.string("evidence") }, args => withDb(args.db, Effect.gen(function*() {
  ...
  yield* Console.log(JSON.stringify({ reviewId: review.reviewId, status: review.status, outcome: review.outcome, limits: yield* reviewLimits(reviewRun(review), review.phase) }))
})))
const reviewNative = Command.make("native", {
  ...
  yield* Console.log(JSON.stringify({ reviewId: review.reviewId, status: review.status, launched: launch, report, limits: yield* reviewLimits(reviewRun(review), review.phase) }))
  if (!launch) return
  ...
  yield* Console.log(JSON.stringify({ reviewId: review.reviewId, status: "awaiting-findings", report, limits: yield* reviewLimits(reviewRun(review), review.phase) }))
})))
