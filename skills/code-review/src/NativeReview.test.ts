...
`, { mode: 0o700 })
  ...
  it("reviews whole-mode committed and local changes as one frozen-base envelope", async () => {
    ...
    try {
      ...
      await execFile("git", ["add", "base.txt", "case.txt", "replacement/tracked.txt"], { cwd: directory })
      await execFile("git", ["commit", "-m", "base"], { cwd: directory })
  ...
  it("keeps base instructions active and exposes target instructions only as review data", async () => {
    ...
    try {
      ...
      await writeFile(join(directory, "base.txt"), "base\n")
      await execFile("git", ["add", "AGENTS.md", "docs/link", "docs/real/rules.md", "nested/AGENTS.md", "scope/AGENTS.md", "scope/controls/rules.md", "case-scope/AGENTS.md", "case-scope/TARGET/rules.md", "base.txt"], { cwd: directory })
      await execFile("git", ["commit", "-m", "base"], { cwd: directory })
      ...
      await writeFile(join(directory, "feature.txt"), "feature\n")
      await execFile("git", ["add", "agents.md", "docs/real/rules.md", "nested", "scope/controls", "hostile-controls/rules.md", "case-scope/target/rules.md", "AGENTS.override.md", ".codex/config.toml", ".agents", "hostile-agent-root/skills/hostile/SKILL.md", ".gitattributes", ".gitignore", "hostile.md", "base.txt", "feature.txt"], { cwd: directory })
