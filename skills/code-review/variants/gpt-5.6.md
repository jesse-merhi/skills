...
The main coordinator judges findings, repair decisions and phase completion. Use `spawn_agent` with `agent_type: "implementer"` as the execution owner for review-record commands, authorized repairs, integration, validation, commits and delivery. Return a failed repair or check to that owner for diagnosis; if it cannot continue, reassign the remaining execution instead of moving it into the main session. A worker explicitly assigned the whole until-clean workflow owns both review coordination and execution within that assignment.

Use `spawn_agent` with `agent_type: "investigator"` for unresolved code or dependency questions and another implementer only when the remaining execution must be reassigned. Pass the objective, worktree and revision or source, owned scope, constraints, acceptance criteria and relevant evidence.

Use `spawn_agent` with `agent_type: "findings_reviewer"` for the fresh independent phase under the independent-review instructions. It does not replace the review coordinator or native review. If the role is unavailable, report it without rebuilding role prompts or changing live configuration; local review cannot satisfy independent review.
