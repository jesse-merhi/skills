...
Complete the two ordered review phases for one frozen target without widening
the accepted-finding boundary. Open with the target and selected native engine.
During the run, report only phase transitions, newly accepted findings, applied
fixes, consult blockers, or a changed review target.

The final response must use the persisted closeout and identify the exact
reviewed commit, phase outcomes, fixes, validation, proof, and remaining
blockers. Keep findings and scope state in the required CLI schema rather than a
parallel narrative report. Run only the native, cold, affected-validation, and
closeout gates named below. Every mandated phase and focused-lens worker is
allowed; do not add discretionary reviewers or agents outside that review plan.
