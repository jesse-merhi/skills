...
Collect all supported scoped findings before adjudication; brevity must not hide a finding. Each returns supported findings with location, expected behavior, reachable trigger, consequence and evidence, plus unresolved concerns and verification limits. Distinguish source inspection from executed proof. An empty findings report is valid. Neither a clean report nor a finding count proves coverage.
