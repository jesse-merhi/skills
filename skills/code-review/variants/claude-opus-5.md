...
4. Run one-time setup for the current target.
   ...
   before Phase 1; do not slice a diff merely because it has many files.
   For discovery briefs, request every genuine scoped candidate with evidence
   instead of the reference's actionable-only output. The coordinator applies
   actionability filtering and records rejections after discovery. Preserve the
   slice limits, coverage records, and prohibition on recursive reviewers. Done
...
7. Run Phase 2.
   ...
   Retain every required reviewer. Run optional lenses locally unless their
   scope is substantial and independent; combine overlapping lenses rather
   than assigning one worker per lens. Ask custom reviewers to collect every
   genuine scoped candidate before actionability filtering. Do not add a final
   verifier or alter the target-only native review command.
