...
Run this workflow only when requested or required by the authorized caller. It permits the exact bot command below on a PR authored by, or substantially contributed to by, the user. Other comments require separate permission. A bot-only run does not run native/cold review or satisfy their gate.
