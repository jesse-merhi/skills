...
- Claude Code: the main coordinator invokes the native `Agent` tool with `subagent_type: "findings-reviewer"` and a task-specific neutral `prompt` when the optional named definition is available; the execution owner cannot use `Agent`. Use a fresh invocation and do not continue or resume it; the definition owns reusable review instructions. Keep implementation rationale and prior findings out of the prompt. Do not assume profile memory or skill-exclusion controls exist. When the named definition is unavailable, use another supported fresh findings-only Claude subagent without a `memory` field. If no fresh dispatch is available, report the cold phase as blocked. Never use this role for native review or an until-clean coordinator. For a separately authorized fresh Claude CLI session, set `CLAUDE_CODE_DISABLE_AUTO_MEMORY=1` for that process only.
...
Wait for that review using `wait-efficiently`. After it returns, the review coordinator checks the findings and the execution owner records them with the review handle, together with the files whose changed behavior was assessed. Finish the review only after every record succeeds. Reading a file for context or checking only one concern does not count as reviewing all its changed behavior. Continue the main review loop.
