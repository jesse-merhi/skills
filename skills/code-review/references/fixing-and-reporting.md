...
Record actionable findings, unresolved concerns and meaningful verified rejections through `record --review <id>` in batches as evidence becomes available, then reconcile the complete report. Omit immediately discarded speculation without individual records or summary counts. Match meaningful repeated reports to the same open or rejected cause with `record --match-of`. Include the revision and why the cause and counterevidence still apply. Changed facts requiring a new judgment, or recurrence after a fix, need a new decision linked to the earlier ID.
