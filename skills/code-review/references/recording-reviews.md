...
Use `reuse` only for an equivalent exact branch patch identity with enough applicable source invocations to meet every configured phase target. Use `focused` with every required phase whose earlier evidence no longer covers the affected behavior; missing required phases are also included. An unaffected phase is carried only when its applicable source invocations meet that phase's configured target. Use `broad` when all configured required phases and previously evidenced phases must run again. Every assessed affected phase remains mandatory even when the run uses default phase requirements. The assessment carries only eligible completed phase evidence and preserves the source invocation IDs. It does not turn inherited evidence into a current-candidate invocation or carry validation command records. Reuse validation only through separately recorded evidence whose relevant behavior and assumptions still apply.
