...
For every changed candidate, the coordinator asks: “Do the changes since the last applicable review invalidate any of its conclusions?” Inspect that delta and the earlier review's findings, assumptions and coverage. Include relevant upstream changes and affected callers, shared contracts, dependencies, configuration and runtime behavior. Examine the complete delta since the reviewed candidate, not only the latest commit when several changes have accumulated.

Use inspection to identify a concrete change in behavior or an unresolved assumption. Run affected tests or another small check when it can answer that question; retain passing results whose inputs still apply. A routine change may be settled by inspection alone. A passing test supports only the behavior it covers, so an untested review concern can still require focused review. Preserve checks explicitly required on the final revision.

Record what changed, which review conclusions remain valid or became uncertain, and the evidence supporting that judgment. Keep this proportional to the change; commit identity, patch equality, file overlap and test success are inputs, not automatic verdicts. Choose one bounded decision:

- Reuse the earlier evidence when the assessment establishes that no required review conclusions were invalidated. The patch may differ; explain why the changed behavior and integration remain covered.
