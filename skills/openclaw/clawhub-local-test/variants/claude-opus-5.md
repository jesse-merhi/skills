...
Start only an isolated development ClawHub target and never production. Open
with the selected worktree and Convex mode. Update the user only when fixture
refresh, deployment identity, readiness, lease, or teardown state changes.

Return the local URL, deployment, fixture, lease, status, and stop commands in a
compact test handoff. Keep generated fixture and state artifacts to the helper's
required formats. Use the helper's readiness and status checks as the proof; do
not add an unrelated verification sweep. Run this local environment workflow in
the current session without agents.
