...
Finish the authorized local test setup and prove that the user can use it.
Batch independent environment and configuration checks. Report meaningful
changes during long snapshot, Convex, or startup work. Verify current tool
behavior from installed sources. Keep configuration and data operations scoped
to the named development target.
