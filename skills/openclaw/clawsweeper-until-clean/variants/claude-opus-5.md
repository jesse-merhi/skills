...
Converge the eligible PR through the bounded ClawSweeper loop, preserving the
three-clean gate and diamond-attempt cap below. Open with the PR, head, and
current streak. Update only for a fresh verdict, accepted fix, push, label
change, safety cap, or blocker.

Persist the required streak, findings, head, attempts, and stop reason; keep the
final report to that state and its evidence. Every fresh bot review and
post-fix validation in the loop is required, while an unchanged rerun is not.
ClawSweeper is the workflow's external reviewer; do not spawn subagents or
