...
```bash
<skill-dir>/scripts/openclaw-local-test --repo ~/repos/openclaw --runtime codex
```

Select the installed Claude CLI and its user settings:

```bash
<skill-dir>/scripts/openclaw-local-test --repo ~/repos/openclaw --runtime claude
```

Override the model when a test needs a specific route:

```bash
<skill-dir>/scripts/openclaw-local-test --runtime codex --model <codex-model>
<skill-dir>/scripts/openclaw-local-test --runtime claude --model <claude-model>
```
...
```bash
<skill-dir>/scripts/openclaw-local-test \
...
```bash
<skill-dir>/scripts/openclaw-local-test --open
<skill-dir>/scripts/openclaw-local-test --open --browser "Google Chrome"
```

Choose a lease or disable channels for a smoke test:

```bash
<skill-dir>/scripts/openclaw-local-test --ttl 30m
<skill-dir>/scripts/openclaw-local-test --no-open --no-channels
```
...
```bash
<skill-dir>/scripts/openclaw-local-test --no-ttl
```

Inspect or stop the managed instance:

```bash
<skill-dir>/scripts/openclaw-local-test --status
<skill-dir>/scripts/openclaw-local-test --stop
```
