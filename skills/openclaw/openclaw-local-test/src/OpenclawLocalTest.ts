...
const RouteReservation = Schema.Struct({ gatewayPid: Schema.Int, proxyPort: Schema.Int })
const RouteReservationJson = Schema.fromJsonString(RouteReservation)
...
export const choosePorts = Effect.fn("Openclaw.choosePorts")(function*(start: number, gateway: Option.Option<number>, proxy: Option.Option<number>, reserved = new Set<number>()) {
  ...
  while (candidate < 65_533) {
    ...
    const portsReserved = reserved.has(candidate) || reserved.has(proxyPort) || reserved.has(candidate + 2)
    if (portsDistinct && !portsReserved && gatewayFree && proxyFree && companionFree) return { gateway: candidate, proxy: proxyPort }
    if (Option.isSome(proxy) && (!proxyFree || reserved.has(proxyPort))) return yield* new LocalTestError({ message: `requested proxy port is busy or reserved: ${proxyPort}` })
...
const health = (url: string) => Effect.tryPromise(() => fetch(url, { signal: AbortSignal.timeout(2_000) })).pipe(Effect.map((response) => response.ok), Effect.catch(() => Effect.succeed(false)))
...
const formatLeaseExpiry = (value: string | undefined) => {
  if (value === undefined || value === "0") return "none"
  const epoch = Number(value)
  return Number.isSafeInteger(epoch) ? new Date(epoch * 1_000).toISOString() : value
}
...
export const openclawLocalTest = Effect.fn("Openclaw.localTest")(function*(raw: OpenclawOptions) {
  ...
  const profileScript = fileURLToPath(new URL("../scripts/runtime-profile.mjs", import.meta.url))
  const proxyScript = fileURLToPath(new URL("./shared-browser-proxy.ts", import.meta.url))
  const baseConfig = Option.map(raw.baseConfig, expandHome)
  ...
  const acquireStartLock = Effect.gen(function*() {
    ...
    for (let attempt = 0; attempt < 600; attempt += 1) {
      ...
      if (Option.isNone(owner) && ownerlessAttempts < 2) {
        ownerlessAttempts += 1
        if (attempt === 0) yield* Console.error(`waiting for OpenClaw startup lock owner: ${startLockDir}`)
  ...
  yield* Effect.forEach([state, workspace, runDir, logs, routes, proxyLogs], (directory) => fs.makeDirectory(directory, { recursive: true, mode: 0o700 }).pipe(Effect.andThen(fs.chmod(directory, 0o700))), { discard: true })
  const startup = Effect.gen(function*() {
  yield* stop
  yield* rotateAgentSessions
  const reservedProxyPorts = new Set<number>()
  for (const entry of yield* fs.readDirectory(routes)) {
    if (!entry.endsWith(".json")) continue
    const routePath = paths.join(routes, entry)
    const route = yield* fs.readFileString(routePath).pipe(Effect.flatMap(Schema.decodeUnknownEffect(RouteReservationJson)), Effect.option)
    if (Option.isNone(route) || !(yield* pidRunning(route.value.gatewayPid))) {
      yield* fs.remove(routePath, { force: true })
      continue
    }
    reservedProxyPorts.add(route.value.proxyPort)
  }
  ...
  const start = Effect.gen(function*() {
    ...
    if (ttl > 0) { const self = fileURLToPath(new URL("../scripts/openclaw-local-test", import.meta.url)); const watchdog = yield* startDetached("sh", ["-c", `sleep ${ttl}; exec \"$1\" --state-dir \"$2\" --stop`, "watchdog", self, state], { stdout: `${logs}/watchdog.log`, stderr: `${logs}/watchdog.err.log` }); yield* fs.writeFileString(files.watchdog, `${watchdog}\n`) }
