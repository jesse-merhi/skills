...
Outcome: provide a ready-to-use local OpenClaw instance for manual browser
testing, with its runtime, state, and credentials isolated from the user's
normal environment.

## Workflow

Use inspected runtime availability and the auto defaults to resolve routine setup choices. Complete the isolated launch and required probes without an extra model-choice interview; preserve native authentication ownership.
