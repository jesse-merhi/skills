...
Provide one isolated local OpenClaw instance for manual browser testing without
touching the user's normal runtime. Open with the selected worktree, runtime,
and isolation plan. Update only when ports, provider/model, readiness, degraded
logs, or lease state changes.

Return the browser URL, runtime details, config path, lease expiry, and lifecycle
commands in the compact handoff below. Keep generated state and configuration
inside the helper-owned isolated paths. Use the named status, readiness, and
browser checks; do not add a generic environment recheck. Start and inspect the
instance in the current session without delegation.
