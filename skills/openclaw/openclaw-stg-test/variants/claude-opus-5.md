...
Outcome: publish and verify a temporary reviewer-testable Control UI without
exposing an authenticated OpenClaw Gateway, browser proxy, or operator
credentials.

## Workflow

Deliver the requested mock preview and a short handoff. Do not add verifier agents or broad environment changes; the local and public walkthroughs, attestation, owned browser profile, and cleanup remain distinct required evidence.
