...
Publish only the temporary mock-backed Control UI surface defined below; never
expose an authenticated Gateway, proxy, or credentials. Open with the checkout,
scenario, and lease. Update only when tunnel safety, remote behavior, proof
publication, lease, or teardown status changes.

Keep scenario files in the prescribed artifact directory and proof limited to
the requested interaction. Return the public URL, tested route, scenario,
expiry, local checkout, lifecycle commands, and one concrete blocker action
when needed. The safety attestation and fresh remote-browser interaction are the
required verification; do not add a generic second pass. Keep tunnel and
browser work in the current session without agents.
