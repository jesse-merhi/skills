...
3. Dispatch a fresh independent reviewer.
   ...
   [references/subagent-dispatch.md](references/subagent-dispatch.md). Build the
   custom brief from its scope and evidence requirements: request every genuine
   scoped candidate before actionability filtering, with accepted findings and
   audit-only rejections returned separately. End after complete changed-flow
   coverage and the verdict; omit the template's additional sweep and recursive
   verifier workers. Preserve each required fresh cold pass. Done when
