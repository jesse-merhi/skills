...
Drive the cold-review phase to its configured clean stop condition and keep all
fixes inside accepted findings. When called by `code-review`, return pass
outcomes, accepted findings, and blockers to that coordinator without taking
over its user-facing final response. On direct invocation, report only a changed
verdict, fix, or stop reason.

Persist findings, scope checks, and the final phase result through the required
findings CLI; do not create a second report artifact. Each fresh cold reviewer
and affected validation run below is part of this workflow, not a redundant
recheck. Dispatch exactly the reviewers the loop requires and no additional
verification agents.
