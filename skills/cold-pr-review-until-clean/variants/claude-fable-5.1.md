...
Finish the authorized cold-review loop through its clean stop condition. Batch
independent reads and checks, and keep the coordinator working while a reviewer
runs when useful. Report each new review result, accepted fix, validation
change, or blocker. Keep fixes and tests limited to accepted findings and use
targeted edits.
