...
Govern the owning until-clean loop through its persisted scope, budgets,
findings, consult queue, and fixed-point rules. This is internal review state:
do not emit progress or a final verdict independently. Return state changes and
required user questions to the owning workflow, which controls communication.

Persist only the required CLI records and optional decision-log rationale; do
not create a parallel report. The deterministic budget, scope, and completion
commands below are the authoritative checks. Apply them in the coordinator
context and do not delegate guardrail decisions.
