...
const forwardsSameIdentifiers = (parameters, arguments_) =>
	parameters.length > 0 &&
	parameters.length === arguments_.length &&
	parameters.every(
		(parameter, index) =>
			identifierName(parameter) !== undefined && identifierName(parameter) === identifierName(arguments_[index]),
	);
...
const isTrivialForwarder = (node) => {
	...
	return (
		...
		!calleeContainsCall(returned.callee) &&
		forwardsSameIdentifiers(node.params, returned.arguments)
	);
};
