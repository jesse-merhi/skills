...
- `version` — `1`.
- `ecosystems.<name>`
  - `detect` — files whose presence at a repository root means the ecosystem is there.
  - `presets` — the key under `presets` holding that ecosystem presets.
  - `packages` — optional. Package name to exact version for tooling the bundled integrations need: `javascript` carries `eslint` itself. Use these pins when selecting those integrations, not to replace an existing native toolchain automatically.
- `presets.<ecosystem>.<preset-name>` — preset columns are keyed by ecosystem name, the same names `ecosystems` uses.
  - `file` — path relative to the skill directory. A config file for single-file presets, a directory for rule sets and check packages.
...
- an empty enforcement array or a kind in the wrong column, such as a shell script under `javascript`.
