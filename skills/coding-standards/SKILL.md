...
Read [catalog-format.md](references/catalog-format.md) before writing anything
into `catalog.json`.
...
Done when `catalog.json` has been read from a resolved real path, and the source
repository URL and commit SHA are both written down. `apply` records them in the
target manifest and `sync` compares against them, so a guessed path silently
produces a manifest that points at nothing.
