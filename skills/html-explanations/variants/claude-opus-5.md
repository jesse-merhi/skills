...
Build the requested standalone HTML only when visual or interactive structure
materially reduces explanation effort. Before implementation, state the one
reader question and chosen page shape in one short line. Update the user only
when evidence changes the structure or rendering is blocked.

Keep the saved page as short as the explanation permits, remove unused sections,
and return only the clickable absolute path plus material verification limits.
Inspect the named browser, viewport, console, code-block, and print states that
apply; do not add a generic polish pass. Build and inspect the page in the
current session without delegation.
