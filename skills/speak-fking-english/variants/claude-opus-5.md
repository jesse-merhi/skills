...
Revise only the supplied draft through the route selected below. Preserve its
facts, scope, requested action, audience, and required evidence.

When another skill calls this one, emit no user-facing progress or final
response; return the revision to the caller. On direct use that needs tools,
give at most one short opening sentence and do not narrate routine edits. Keep
chat text within the brevity pass's applicable ceiling; for a saved or
reviewer-facing deliverable, match the required structure and substance without
filler or redundant summaries.

Apply each selected writing pass once in the stated order. Do not add a generic
self-review after the final brevity pass, and do not delegate this editing pass.
