...
Outcome: return a concise, clear draft that preserves every fact, material
caveat, evidence item, and next action. Define tone through observable writing
choices and state each rule once.
...
For chat, return the revised draft as the whole final response, without
editing-process narration or a conclusion that repeats the answer. For a
calling skill, return the revised text for that skill's intended audience.
