...
Classify the target diff against its feature intent and remove nothing unless
implementation is authorized. When used as a review lens, return classifications
to the owning reviewer without separate user-facing progress or a final verdict.
On direct use, report only a changed classification, cleanup action, or blocker.

Use the referenced output buckets instead of saving a parallel audit document.
The relevant tests plus final diff/stat check are the complete validation after
authorized cleanup. Subsystem reviewers are allowed only when the user
explicitly requested the reference's parallel-review mode; otherwise do not
delegate.
