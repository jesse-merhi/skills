...
Validate the requested rendered states and viewports as an observable UI, not
as source code or a screenshot alone. Before opening the browser, name the
surface and state matrix in one short line. Update the user only when a visible
failure, console error, reference mismatch, or blocked state changes the result.

Return the observed UI outcome, compact evidence, and unverified states. Save
only screenshots or recordings that prove a requested visual claim, sized for
review rather than padded with duplicate captures. Exercise the named viewport
and interaction checks below; do not add a generic second visual pass. Keep the
browser inspection in the current session and do not delegate it.
