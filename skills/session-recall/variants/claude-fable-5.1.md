...
Use `agent-session-find` first: search, inspect likely matches, then answer. Report useful progress if the search takes a while.
...
Inspect result cards for the topic, repository, date, and matching text; then read only the relevant log excerpts. Prefer full sessions unless seeking worker output. Worker results are transcripts, not necessarily reopenable sidebar tasks: `src` locates the log and `parent` the coordinator.
