...
Complete the recall with the smallest useful local transcript exposure. Batch
independent bounded queries after the first index refresh. During a long search,
report a changed match or blocker. Quote copied session wording and distinguish
it from summary. Keep session history local and append-only; never edit or
upload the logs.
