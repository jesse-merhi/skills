...
Remove only local artifacts whose ownership and disposable state are proved by
the inventory below. Open with one short line naming the cleanup target. Update
the user when the inventory exposes a decision item, deletion scope changes, or
cleanup becomes blocked.

Keep the inventory and final report to removed, retained, unresolved, and remote
state. Do not create a separate cleanup document unless requested. Verify
removal by repeating the same identity-based discovery queries; additional
generic scans do not strengthen the proof. Cleanup decisions and destructive
actions stay with the current agent and are never delegated.
