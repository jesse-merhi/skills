...
Ground the upcoming `grilling` session in current project evidence, then let
that interview workflow own the user dialogue. Give one short grounding update
only when repository or Obsidian lookup will take time; otherwise begin the
first question round. Do not add separate progress narration between rounds.

Return the tightened plan or blocker set required below. Save no additional
document unless the user requests one. Source checks support the questions and
recommendations; they are not a separate verification phase. Any fact-finding
delegation belongs to the loaded `grilling` workflow, not this wrapper.
