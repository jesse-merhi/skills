...
ADRs live in the user's Obsidian-backed project notes unless the user explicitly asks for repo-local docs.
...
If write access or the target path is unclear, stop and tell the user.
...
{Briefly state the context, decision, and why.}
```

A single paragraph is enough when it records the decision and why.
...
Scan the target `ADRs/` directory for the highest existing number and increment it by one.
...
If a decision is easy to reverse, skip it. If it is obvious, skip it. If there was no real alternative, skip it.
