...
evidence version changes, or repair is blocked.

Keep the delegated recommendation and final repair record compact, redacted,
and tied to the captured invariant. Report the approved change, exact target,
validation, and anything unresolved. Run only the recommendation, approval,
implementation, and invariant validation stages below. The single cold
