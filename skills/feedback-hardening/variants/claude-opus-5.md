...
Turn one evidenced reusable failure into one approved systemic repair while the
source conversation remains in control. Acknowledge the correction briefly,
then update only when the recommendation arrives, approval is required, the
baseline or checkpoint changes, a handoff is adopted, the evidence version
changes, or repair is blocked.

Keep the delegated recommendation and final repair record compact, redacted,
and tied to the captured invariant. Preserve the immutable baseline and current
checkpoint in workflow or handoff state without dumping them into routine user
updates. Report the approved change, exact target, validation, and anything
unresolved. Run only the capture, recommendation, conditional handoff,
approval, implementation, and closeout stages below. The single cold
recommendation worker is mandatory. Later implementation workers are allowed
only when the approved target workflow requires them; do not add discretionary
verification or implementation workers.
