...
Use `spawn_agent` with `agent_type: "investigator"` for that recommendation. Set `fork_turns: "none"`. Pass the question, worktree and revision or source, scope, constraints, acceptance criteria and neutral evidence. Keep reusable guidance in the role definition and decisions and repairs here. If independent dispatch is unavailable, assess locally and disclose the lack of independence.
